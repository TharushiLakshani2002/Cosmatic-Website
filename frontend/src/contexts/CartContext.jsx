...
export const CartProvider = ({children}) => {
    ...
    useEffect(() => {
        const savedCart = localStorage.getItem('fashionhunt_cart');
        if (savedCart) {
            try {
                ...
                localStorage.removeItem('fashionhunt_cart');
            }
        }
    }, []);
    ...
    useEffect(() => {
        if (items.length > 0) {
            localStorage.setItem('fashionhunt_cart', JSON.stringify(items));
        } else {
            localStorage.removeItem('fashionhunt_cart');
        }
